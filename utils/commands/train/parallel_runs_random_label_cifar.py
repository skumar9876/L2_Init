...
def create_all_commands():
    ...
    ENV_ARGS="env=random_label_cifar env.params.unique_samples_per_dataset=1200 \
        env.params.concept_duration=30000 env.params.num_concept_shifts=50 \
        env.params.env_batch_size=16"
    ...
    params = {
        'main.seed': [str(i) for i in range(3)],
        'agent.params.fraction_to_remove': [str(0.28)],  # 0.25 when hidden dim = 10, 0.27 when hidden_dim = 100
    }
    params = {
        **params,
        **lr_params,
    ...
    params = {
        'main.seed': [str(i) for i in range(3)],
        'agent.params.recycle_period': [30000, 60000],
